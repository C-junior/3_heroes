...
func attack(p_target: Node2D):
	if p_target and p_target.has_method("take_damage"):
		_shoot_magic_bolt(p_target)
		attack_timer.start()

# Shoot a projectile towards the target
func _shoot_magic_bolt(p_target: Node2D):
	...
	print("Wizard shot magic bolt at target:", p_target.name)
