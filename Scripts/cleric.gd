...
func _process(_delta: float):
	...
	if injured_ally:
		move_and_heal(injured_ally, _delta)
	use_skills()
...
func move_and_heal(p_target: Node2D, _delta: float):
...
func heal(p_target: Node2D):
	if p_target.has_method("receive_heal"):
		p_target.receive_heal(heal_amount)
